...
The page's "Troubleshooting" section: https://archidep.ch/2026/course/104-hello-ssh/#troubleshooting

Its entries, with their anchors on that page:

- `Please type 'yes', 'no' or the fingerprint:` (#please-type-yes-no-or-the-fingerprint)
- `WARNING: REMOTE HOST IDENTIFICATION HAS CHANGED!` (#warning-remote-host-identification-has-changed)
- I am still asked for my password after `ssh-copy-id` (#i-am-still-asked-for-my-password-after-ssh-copy-id)
- `scp` finishes without error, but nothing arrives on the server (#scp-finishes-without-error-but-nothing-arrives-on-the-server)
- My SFTP application asks for a password or refuses my key (#my-sftp-application-asks-for-a-password-or-refuses-my-key)
- `zsh: no matches found` with `ssh-keygen -R` (#zsh-no-matches-found-with-ssh-keygen--r)
- `ssh -p` ends with `Connection timed out` (#ssh--p-ends-with-connection-timed-out)
