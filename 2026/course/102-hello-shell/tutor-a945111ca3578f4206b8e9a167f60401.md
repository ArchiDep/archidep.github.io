...
The page's "Troubleshooting" section: https://archidep.ch/2026/course/102-hello-shell/#troubleshooting

Its entries, with their anchors on that page:

- The command that prepares the hunt fails (#the-command-that-prepares-the-hunt-fails)
- My shell’s configuration file lost its content (#my-shells-configuration-file-lost-its-content)
- My terminal seems frozen (#my-terminal-seems-frozen)
- `treasure` is still not found after I changed my `PATH` (#treasure-is-still-not-found-after-i-changed-my-path)
- Every command is now `command not found` (#every-command-is-now-command-not-found)
